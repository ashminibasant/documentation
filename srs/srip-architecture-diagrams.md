---
title: SRIP Architecture Diagrams
description: Non-normative diagrams for selected SRIP architecture relationships.
published: true
...
These diagrams render selected integration views of
`srip-dependency-graph.yaml` using the evidence classifications in
`srip-dependency-reviews.yaml`. They do not attempt to render every canonical
parent edge; complete canonical coverage remains in the graph.
...
## 6. Trajectory Measurement And Admission

```mermaid
flowchart LR
    C["Generated candidate"]
    AEP["SRIP-10<br/>Dynamic stability evidence"]
    ADP["SRIP-15<br/>Perturbation / return path"]
    RCB["SRIP-19<br/>Buffer / recovery evidence"]
    TMC["SRIP-27<br/>Trajectory membership"]
    TAL["SRIP-28<br/>Admission transaction"]
    MIL["SRIP-26<br/>Memory influence"]
    D["Selected delivery"]
    H["Hold / reject / review"]

    C --> TMC
    AEP -->|dynamic evidence| TAL
    ADP -->|return-path evidence| TMC
    RCB -->|hold / recovery evidence| TAL
    TMC -->|membership + validity| TAL
    TAL -->|admit one lineage| D
    TAL -->|contain| H
    TAL -->|selected lineage only| MIL

    classDef evidence fill:#e8f1ff,stroke:#3167a8,color:#13233a;
    classDef authority fill:#fff4d6,stroke:#9a6a00,color:#362500;
    classDef outcome fill:#e9f7ef,stroke:#26734d,color:#102d20;
    class AEP,ADP,RCB,TMC evidence;
    class TAL authority;
    class MIL,D,H outcome;
```
...
generated candidate != admitted delivery
TMC measures; TAL admits
```

TAL is the single pre-persistence writer, but it remains constrained by safety,
authorization, governance, and provider-boundary authority.
...
1. canonical SRIP metadata;
2. `srip-dependency-graph.yaml`;
3. `srip-dependency-reviews.yaml`;
4. `srip-relationship-audit.md`;
5. this diagram document;
6. the prose synthesis and relationship matrix if their interpretation changed.
