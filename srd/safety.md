...
- self-model evidence is not proof of consciousness or independent intent;
- reflective artifacts must not be exposed as private system truth;
- RSM must not change safety policy, identity, architecture, or model weights by itself;
- reflection depth and frequency must remain bounded;
- intervention proposals must pass through the relevant control and safety layers.
...
- reject exchange when consent or authorization is missing;
- quarantine imported material with incomplete provenance;
- prevent unstable drift from propagating between runtimes;
- preserve local identity and role boundaries;
- log accepted, narrowed, quarantined, and rejected exchange events.
